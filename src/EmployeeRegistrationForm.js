...
function EmployeeRegistrationForm() {
  ...
  const validate = () => {
    ...
    if (!formData.emailId || !String(formData.emailId).match(/^[\w-.]+@([\w-]+\.)+[\w-]{2,4}$/)) {
      validationErrors.emailId = "Enter a valid email address.";
    }
    ...
    if (!(formData.emergencyContactRelationship || '').trim()) {
      validationErrors.emergencyContactRelationship = "Emergency contact relationship is required.";
    } else if (!/^[A-Za-z\s]+$/.test(formData.emergencyContactRelationship)) {
      validationErrors.emergencyContactRelationship = "Must contain only letters.";
    }
    ...
    if (!formData.bankAccountNumber || !String(formData.bankAccountNumber).match(/^\d{9,18}$/))
      validationErrors.bankAccountNumber = "Enter a valid account number.";
    
   if (!formData.bankIFSC) {
      validationErrors.bankIFSC = "Enter a valid IFSC code.";
    } else if (!/^[A-Z]{4}0[A-Z0-9]{6}$/.test(formData.bankIFSC)) {
      validationErrors.bankIFSC = "Enter a valid IFSC code (only uppercase letters and numbers, e.g., ABCD0E12345).";
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch("http://localhost:8091/api/employee", {
        method: "POST",
          ...
          <div className="form-grid">
            ...
            <div className="form-group">
              ...
              <input id="emailId"
                ...
                className={`${errors.emailId ? "input-error" : formData.emailId ? "input-valid" : ""} ${shakeFields.email ? 'shake' : ''}`}
                autoComplete="off"
              />
              {errors.emailId &&
                <span className={`error validation-msg${fadingErrors.emailId ? " fade-out" : ""}`}>
                  {errors.emailId}
                </span>
              }
            </div>
          ...
          <div className="form-grid">
            ...
            <div className="form-group">
              ...
              <input id='emergencyContactRelationship'
                ...
                onChange={e => {
                  const filtered = e.target.value.replace(/[^A-Za-z\s]/g, "");
                  setFormData(prev => ({ ...prev, emergencyContactRelationship: filtered }));
                  triggerFadeOut('emergencyContactRelationship');
                }}
                className={`${errors.emergencyContactRelationship ? "input-error" : formData.emergencyContactRelationship ? "input-valid" : ""} ${shakeFields.emergencyContactRelationship ? 'shake' : ''}`}
                autoComplete="off"
              />
              {errors.emergencyContactRelationship &&
                <span className={`error validation-msg${fadingErrors.emergencyContactRelationship ? " fade-out" : ""}`}>
                  {errors.emergencyContactRelationship}
                </span>
              }
            </div>
            ...
            <div className="form-group">
             ...
             <input id='bankAccountNumber'
                ...
                onChange={e => {
                  // Only allow letters
                  const filtered = e.target.value.replace(/[^\d]/g, "");
                  setFormData(prev => ({ ...prev, bankAccountNumber: filtered }));
                  triggerFadeOut('bankAccountNumber')
                }}
                className={`${errors.bankAccountNumber ? "input-error" : formData.bankAccountNumber ? "input-valid" : ""} ${shakeFields.accountNumber ? 'shake' : ''}`}
                autoComplete="off"
                maxLength={18} // Assuming max length for account number is 18 digits
              />
             
              {errors.bankAccountNumber &&
                <span className={`error validation-msg${fadingErrors.bankAccountNumber ? " fade-out" : ""}`}>
                  {errors.bankAccountNumber}
                </span>
              }
            </div>
            <div className="form-group">
              ...
              <input id='bankIFSC'
                  ...
                  onChange={e => {
                    ...
                    setFormData(prev => ({ ...prev, bankIFSC: filtered }));
                    triggerFadeOut('bankIFSC');
                  }}
                  maxLength={11}
                  className={`${errors.bankIFSC ? "input-error" : formData.bankIFSC ? "input-valid" : ""} ${shakeFields.ifscCode ? 'shake' : ''}`}
                  autoComplete="off"
                />
              {errors.bankIFSC &&
                <span className={`error validation-msg${fadingErrors.bankIFSC ? " fade-out" : ""}`}>
                  {errors.bankIFSC}
                </span>
              }
            </div>
